  ...
  return (
    <div className="group bg-white rounded-2xl shadow-md border border-gray-100 p-5 flex flex-col items-center text-center transform transition-all duration-300 hover:-translate-y-2 hover:shadow-xl">
      ...
      <p className="mt-2 text-sm text-gray-600">
        মাসিক অনুদান: <span className="font-bold text-green-600">৳{member.monthly_donation?.toLocaleString()||0}</span>
      </p>
      {member.location && <p className="text-xs text-gray-400 mt-1">{member.location}</p>}
      <Link
        to={`/member/${member.id}`}
      ...
      {member.contact && <a href={`tel:${member.contact}`} className="mt-2 text-sm text-green-700 hover:text-green-900 transition-colors duration-300">যোগাযোগ করুন</a>}
    </div>
  );
}
